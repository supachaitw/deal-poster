...
- ทางเข้าเว็บ deals. จาก Home Console (26 ก.ย. 69): การ์ด **"เว็บดีลสาธารณะ"** ใน `index.html` (TILES) · ปุ่ม **🌐 เปิดเว็บดีล** บนหัวหน้า `dealposter.html` · ลิงก์ **เว็บ ↗** รายแถว (โพสต์แล้ว+ไม่ซ่อน) ไป `/d/<id ไม่มีขีด>` — ไฟล์ทั้งสองอยู่ใน repo `home-console` (`live/html/`) ต้องเอาไป commit ที่นั่นด้วย
