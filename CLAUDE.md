...
- **ความคืบหน้า 30 ส.ค. 69**: สมัคร Partner Center ด้วย**อีเมลใหม่** (อีเมลเดิม gmail ผูกร้าน TikTok Shop → App developer ถูกล็อก "not available for TikTok Shop sellers") · Partner name **Paiyaa Deals**, region Thailand
  ...
  - **บล็อกอยู่ (30 ส.ค. 69)**: หน้า creator authorize ขึ้น **"Invalid app key"** ทั้งที่ token endpoint รู้จักแอป (ยิง auth_code ปลอมได้ `36004004 invalid auth code` = คู่ key/secret ใช้ได้) → สรุปว่าแอปยังไม่ถูกปล่อยรับ authorization เพราะ **qualification "Creator collaborations" ยัง Draft ไม่ได้ submit** — และ **user ไม่มีทะเบียนพาณิชย์** จึง submit Certification ไม่ได้ · ทางออก: ยื่น ticket ถาม Partner Assistant (ร่างข้อความให้แล้วใน session log — ถามเรื่อง individual developer path ด้วย) · แผนสำรอง: จดทะเบียนพาณิชย์บุคคลธรรมดา (~50 บาท) แต่เงื่อนไขเขียนว่ากิจการต้องจดเกิน 1 ปี
